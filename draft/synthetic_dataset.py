...
for subject in subject_list:
    for verb in verb_list:
        for adjective in adjective_list:
            ...
            df_output_2.loc[len(df_output_2)] = {'Sentences': temp_sentence, 'Logic-words': 'None'}
            df_output.loc[len(df_output)] = {'Sentences': temp_sentence, 'Logic-words': 'positive'}
            
            temp_sentence_negation = subject + " " + verb + " not " + adjective + "."
            df_output.loc[len(df_output)] = {'Sentences': temp_sentence_negation, 'Logic-words': 'negative'}
...
for index, item in df_output.iterrows():
    ...
    df_output_if_then.loc[len(df_output_if_then)] = {'Sentences': temp_sentence, 'Logic-words': 'if,then'}
...
for index, item in df_output.iterrows():
    ...
    df_output_and_or.loc[len(df_output_and_or)] = {'Sentences': temp_sentence, 'Logic-words': 'and'}

    if "not" in sentence1 and "not" not in sentence2:
        temp_sentence = sentence2[:-1] + " or " + sentence1 + "."
    else:
        temp_sentence = sentence1 + " or " + sentence2
    df_output_and_or.loc[len(df_output_and_or)] = {'Sentences': temp_sentence, 'Logic-words': 'or'}


df_output = pd.concat([df_output, df_output_if_then], ignore_index=True)
df_output = pd.concat([df_output, df_output_and_or], ignore_index=True)
